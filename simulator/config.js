...
export function createConfig(overrides = {}) {
    ...
    if (!Number.isInteger(config.boardSize) || config.boardSize < 3) {
        throw new Error("boardSize must be an integer of at least 3.");
    }
    if (!Number.isInteger(config.symbolCount) || config.symbolCount < 3) {
        throw new Error("symbolCount must be an integer of at least 3.");
    }
    if (!Number.isInteger(config.queueSize) || config.queueSize < 1) {
        throw new Error("queueSize must be an integer of at least 1.");
    }
    if (!Number.isInteger(config.startingScore) || config.startingScore < 1) {
        throw new Error("startingScore must be an integer of at least 1.");
    }
